/*
 * File: glmRoutes.ts
 * Z.AI GLM provider handler for /v1/chat/completions.
 *
 * Mirrors deepseekRoutes.ts: GLM needs no browser, no account rotation and no
 * session handling. Streaming and non-streaming responses are proxied to the
 * client, and usage is billed against the gateway API key like any other request.
 */

import type { Context } from 'hono';
import { callGlm, GLM_MODELS } from '../services/glm.ts';
import { config } from '../services/configService.ts';
import { logStore } from '../services/logStore.ts';
import { recordApiKeyUsage } from '../services/apiKeyStore.ts';
import type { OpenAIRequest } from '../types/openai.ts';

export interface GlmHandlerArgs {
  c: Context;
  logId: string;
  body: OpenAIRequest;
  messages: unknown[];
...
/** Extract usage numbers from a GLM response (stream or JSON). */
...
function bill(c: Context, prompt: number, completion: number): void {
  ...
  try {
    ...
    logStore.log('warn', 'glm', `Failed to record usage: ${err instanceof Error ? err.message : String(err)}`);
  }
}

/** Shared upstream call, mapping our internal shape onto the provider. */
async function callUpstream(args: GlmHandlerArgs, signal: AbortSignal) {
  const { body, messages, isStream } = args;
  return callGlm({
    apiKey: config.get('GLM_API_KEY') || '',
    model: body.model,
    messages,
    stream: isStream,
    tools: body.tools,
...
async function handleNonStreaming(args: GlmHandlerArgs, signal: AbortSignal) {
  const { c, logId } = args;
  const result = await callUpstream(args, signal);

  if (!result.ok || !result.response) {
    const err = result.error!;
    logStore.log('error', 'glm', `[GLM] ${err.code}: ${err.message}`);
...
/**
 * Streaming: pipe GLM's SSE to the client verbatim while sniffing the usage
 * chunk so billing stays accurate. The stream is NOT rewritten.
 */
async function handleStreaming(args: GlmHandlerArgs, signal: AbortSignal) {
  const { c, logId } = args;
  const result = await callUpstream(args, signal);
  ...
  const stream = new ReadableStream<Uint8Array>({
    async start(controller) {
      ...
      try {
        ...
        if (!finalized) {
          logStore.log('warn', 'glm', `[GLM] stream error: ${err instanceof Error ? err.message : String(err)}`);
...
/**
 * Entry point for GLM-backed requests. Chooses streaming vs buffered and wires
 * client-abort handling.
 */
export async function handleGlmRequest(args: GlmHandlerArgs) {
  const { c, logId, body } = args;
  const model = GLM_MODELS.find((m) => m.id === body.model);

  logStore.log(
    'info',
    'glm',
    `[GLM] model=${body.model} stream=${args.isStream} msgs=${args.messages.length}${model ? ` (${model.label})` : ''}`,
  );
  ...
  try {
    ...
    logStore.log('error', 'glm', `[GLM] unexpected failure: ${message}`);
    logStore.finalizeRequest(logId);
    return c.json(
      { error: { message: `GLM request failed: ${message}`, type: 'upstream_error', param: null, code: 'glm_failed' } },
      502 as never,
    );
  }
}
