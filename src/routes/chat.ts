...
export async function chatCompletions(c: Context) {
  ...
  try {
    ...
    // ── Auto-retry callback ──────────────────────────────────────────
    // When an upstream stream times out or fails mid-response, this
    // function picks a fresh account/session/stream so the SSE client
    // (Qwen Code CLI) never sees the failure — it just continues.
    const retryStream = async (lastFailedEmail?: string) => {
      const RETRY_MAX = 3;
      let skipEmail = lastFailedEmail || resolvedEmail;
      for (let i = 0; i < RETRY_MAX; i++) {
        const account = await pickAccount(skipEmail);
        if (!account?.email) return null;
        try {
          const sr = await acquireSessionWithCorrections(account.email, sessionMessages);
          const rm = await modelRouter.route(body.model);
          const str = await createQwenStreamWithRetry(
            sr.qwenMessages,
            isThinkingModel,
            rm,
            sr.session.chatId,
          ...
          logStore.log('debug', 'chat', `[Chat] Auto-retry routed to ${sr.resolvedEmail}`);
          return {
            stream: str.stream,
            qwenAbortController: str.abortController,
            resolvedEmail: sr.resolvedEmail,
            session: sr.session,
            initialParentId: sr.nextParentId,
            sessionHeaders: sr.sessionHeaders,
          };
        } catch (err: any) {
          logStore.log('warn', 'chat', `[Chat] Auto-retry account ${account.email} failed: ${err.message}`);
          skipEmail = account.email;
          if (err.upstreamStatus === 429) continue;
          if (err.message?.includes('FAIL_SYS_USER_VALIDATE') || err.message?.includes('CAPTCHA')) {
            throttleAccount(account.email, 5 * 60 * 1000);
            continue;
          }
          continue;
        }
